...
MoonMCP exposes **172 tools**, **11 resources** and **9 operator prompts**, grouped by how much they touch the target:
...
| `search_tools` | **Find the few tools relevant to what you're doing** instead of scanning all ~172 — keyword/phrase in (`"graphql"`, `"jwt"`, `"cache poisoning"`), a short ranked list out (name match > family > gist). Progressive discovery for a large tool surface. |
...
```
moonmcp/
├── server.py        # FastMCP server: 172 tools, 11 resources, 9 prompts (@active_tool = the one scope gate)
