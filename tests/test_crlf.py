...
def test_inject_raw_appends_without_reencoding():
    ...
    u = crlf.inject_raw("https://x.test/p?a=1", "q", "v%0d%0aX-Crlf-Probe-Inj:1")
    assert u == "https://x.test/p?a=1&q=v%0d%0aX-Crlf-Probe-Inj:1"
...
async def test_probe_confirms_header_injection():
    ...
    def handler(url):
        if "X-Crlf-Probe-Inj:1" in url:
            return _R(200, headers={"X-Crlf-Probe-Inj": "1"})
        return _R(200)
...
async def test_probe_confirms_cookie_split():
    def handler(url):
        if "Set-Cookie:crlfprobe=1" in url:
            return _R(200, set_cookies=["crlfprobe=1; Path=/"])
        return _R(200)
