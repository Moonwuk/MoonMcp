...
async def test_mass_assignment_flags_reflected_privileged_field():
    ...
    res = await logic.probe_mass_assignment(_Client(handler), "https://x.test/api/users", dry_run=False)
    assert any(f["field"] == "role" and f["severity"] == "high" for f in res)


@pytest.mark.asyncio
async def test_race_probe_flags_multiple_successes():
    res = await logic.probe_race(_Client(lambda u, m, b: _R(200)), "https://x.test/coupon", n=6, dry_run=False)
    assert res["success_2xx"] == 6 and res["verdict"] == "review"

    res2 = await logic.probe_race(_Client(lambda u, m, b: _R(429)), "https://x.test/coupon", n=6, dry_run=False)
