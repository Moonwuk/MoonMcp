...
async def test_probe_thinkphp_confirmed_via_md5_echo():
    expected = hashlib.md5(b"x7k2q9").hexdigest()  # noqa: S324

    def handler(url, headers):
        if "invokefunction" in url and "vars[1][]=x7k2q9" in url:
            return _R(200, f"header {expected} footer")
        return _R(404, "")
    # The ThinkPHP RCE-echo probe is opt-in (include_rce_probes=True) since it
    # sends a real call_user_func_array primitive the server executes.
    res = await stacks.probe_stack(_Client(handler), "https://t.test/",
                                   include_rce_probes=True)
    assert any(f["issue"] == "ThinkPHP invokefunction RCE" and f["verdict"] == "confirmed"
               for f in res.findings)


@pytest.mark.asyncio
async def test_probe_thinkphp_skipped_without_consent_flag():
    """Without include_rce_probes the ThinkPHP RCE-echo probe must NOT run."""

    def handler(url, headers):
        if "invokefunction" in url:
            return _R(200, "should not reach here")
        return _R(404, "")
    res = await stacks.probe_stack(_Client(handler), "https://t.test/")
    assert not any(f["issue"] == "ThinkPHP invokefunction RCE" for f in res.findings)
