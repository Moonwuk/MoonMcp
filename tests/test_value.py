...
async def test_coupon_reuse_verdict():
    ok = await v.probe_coupon_reuse(_Seq(_R(200), _R(200)), "https://x.test/apply", "coupon", "SAVE", times=3, dry_run=False)
    ...
    no = await v.probe_coupon_reuse(_OnceThen409(), "https://x.test/apply", "coupon", "SAVE", times=3, dry_run=False)
