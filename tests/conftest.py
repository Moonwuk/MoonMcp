...
def fresh_context(monkeypatch):
    ...
    The intrusive tools (sqli/ssti/lfi/ssrf/... probes) are gated behind
    ``allow_intrusive``. The dataclass now defaults to ``False`` (safe posture,
    audit 2026-07-27 debt #4), and the launcher enables it per-engagement. The
    tests exercise the intrusive probes directly, so the fixture mirrors an
    authorised engagement and turns the gate ON here. ``Settings`` is a frozen
    dataclass, so we build a replacement instead of mutating in place.
    """
