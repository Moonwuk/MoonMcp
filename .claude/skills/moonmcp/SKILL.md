...
| **Injection on a discovered param** *(intrusive)* | `ssti_probe`, `sqli_probe` (context/oob/time/json-waf/multibyte/header lanes), **`cmdi_probe`** (blind OS command injection — separator × sleep/OAST, never reads command output), **`lfi_probe`** (path traversal — depth-escalating/null-byte/double-encoded, confirmed by a real `/etc/passwd`/win.ini signature), `ssrf_probe` (start `oast_selfhost` first), `cache_probe`; WAF blocks the payload? `parser_diff_probe` is the **bypass multiplier** |
