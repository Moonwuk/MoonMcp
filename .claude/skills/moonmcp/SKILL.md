...
| **Auth'd IDOR / BOLA** | `auth_set` first, then `authz_probe` (`direct_only=True` for a single-request diff) |
...
| **A captured SAML SSO login (SAMLResponse + ACS URL)** | **`saml_xsw_probe`** — clones the signed assertion, forges its identity, splices it in via first-wins/last-wins/direct-children-only topologies; `reflected_forged_identity` (forged identity in a variant's response but neither baseline's) is the strong signal, replay-protection-independent |
| **A suspicious cookie/header/hidden-field value** | **`deserialize_fingerprint`** (Freddy-lite) — passive byte/base64 signature scan for Java serialization, .NET ViewState, PHP `serialize()`, Python pickle, Ruby Marshal, Fastjson/Jackson `@type` — reports the format, never invokes a gadget chain (→ ysoserial/PHPGGC/ViewGen via Strix) |
...
| **Injection on a discovered param** *(intrusive)* | `ssti_probe`, `sqli_probe` (context/oob/time/json-waf/multibyte/header lanes), **`cmdi_probe`** (blind OS command injection — separator × sleep/OAST, never reads command output), **`lfi_probe`** (path traversal — depth-escalating/null-byte/double-encoded, confirmed by a real `/etc/passwd`/win.ini signature), **`xxe_probe`** (format-confusion JSON/form→XML rewrite + blind XXE via OAST, never reads file contents), `ssrf_probe` (start `oast_selfhost` first), `cache_probe`; WAF blocks the payload? `parser_diff_probe` is the **bypass multiplier** |
| **Not sure which injection class fits — param behavior is unclear** | **`interp_probe`** (Backslash Powered Scanner-style meta-probe — five character-level markers, ≥2 corroborating before anything more than "weak"; never asserts a class, `suggested_next` names which class-specific probe above to run) |
