...
WAF_ENTRIES: list[dict] = [   {   'id': 'hiw-aws-waf',
    ...
    # 2026-07-24 survey: CN WAF vendors
    {   'id': 'fp-aliyun-waf',
        'name': 'Fingerprint: Alibaba Cloud WAF (阿里云WAF)',
        'category': 'fingerprint',
        'summary': 'Alibaba Cloud Web Application Firewall — CN cloud WAF vendor, identified by '
                   'specific response headers and block pages.',
        'detail': 'Alibaba Cloud WAF is the dominant CN cloud WAF. It proxies traffic through '
                  'Alibaba Cloud infrastructure and applies managed rules + ML-based detection. '
                  'Block pages typically include Chinese-language error messages and Alibaba '
                  'Cloud branding. The WAF integrates with Alibaba Cloud CDN and Anti-Bot service.',
        'applies_to': ['Alibaba Cloud WAF', '阿里云WAF', 'CN cloud WAF'],
        'indicator': "Block body containing '阿里云' or 'Alibaba Cloud' + 'WAF' or '安全'; "
                     "response headers like 'X-Alibaba-WAF' or server 'AliyunWAF'; block status "
                     "often 405/406/418.",
        'references': ['https://www.alibabacloud.com/product/waf']},
    {   'id': 'fp-tencent-waf',
        'name': 'Fingerprint: Tencent Cloud WAF (腾讯云WAF)',
        'category': 'fingerprint',
        'summary': 'Tencent Cloud Web Application Firewall — CN cloud WAF, identified by Tencent '
                   'security headers and Chinese-language block pages.',
        'detail': 'Tencent Cloud WAF is the #2 CN cloud WAF. It integrates with Tencent CDN and '
                  'Anti-DDoS. Block pages include Tencent branding and Chinese error messages.',
        'applies_to': ['Tencent Cloud WAF', '腾讯云WAF', 'CN cloud WAF'],
        'indicator': "Block body containing '腾讯云' or 'Tencent' + '安全' or 'WAF'; response "
                     "headers like 'X-Tencent-WAF' or 'TencentWAF'.",
        'references': ['https://cloud.tencent.com/product/waf']},
    {   'id': 'fp-safeline',
        'name': 'Fingerprint: SafeLine / 雷池 (Chaitin)',
        'category': 'fingerprint',
        'summary': 'AI semantic-analysis WAF by Chaitin (长亭科技) — claims to capture 0-day '
                   'attacks without signatures using semantic analysis. Changes bypass calculus.',
        'detail': 'SafeLine (雷池) is a CN-built WAF by Chaitin Tech (长亭科技, also authors of '
                  'xray passive scanner). It uses AI semantic analysis to understand request intent '
                  'rather than signature matching. Defensive claim: captures 0-day without '
                  'signatures. Offensive implication: traditional payload obfuscation (encoding, '
                  'case-swap, comment-break) may FAIL against semantic engines; need parser-'
                  'differential, logic-layer, or protocol-level bypasses instead. Available as '
                  'community edition and enterprise. Marketed heavily against AI-agent-driven '
                  'attack surge in 2026.',
        'applies_to': ['SafeLine', '雷池', 'Chaitin', 'AI semantic WAF'],
        'indicator': "Block body containing '雷池' or 'SafeLine' or 'chaitin'; block status often "
                     "403 with Chinese semantic-analysis explanation.",
        'references': ['https://safeline.pro/', 'https://chaitin.com/']},
    {   'id': 'fp-anquangou',
        'name': 'Fingerprint: 安全狗 (AnDog / SafeDog)',
        'category': 'fingerprint',
        'summary': 'Host-based WAF (安全狗) widely deployed on CN servers — identified by '
                   'specific block patterns and Chinese error pages.',
        'detail': '安全狗 (AnDog/SafeDog) is a host-based WAF/IPS popular in the CN server '
                  'market. It operates at the OS/web-server level rather than as a cloud proxy. '
                  'Block pages include 安全狗 branding. Known bypass techniques include case '
                  'variation, double-writing, event-function transformation.',
        'applies_to': ['安全狗', 'AnDog', 'SafeDog', 'CN host-based WAF'],
        'indicator': "Block body containing '安全狗' or 'SafeDog' or 'anquangou'; often 403 with "
                     "Chinese security warning.",
        'references': ['https://www.safedog.cn/']},
    # 2026-07-24 survey: RU WAF vendors
    {   'id': 'fp-solidwall',
        'name': 'Fingerprint: SolidWall (Solar Security)',
        'category': 'fingerprint',
        'summary': 'Russian ML-based WAF by Solar Security (SolidSoft) — identified by specific '
                   'response headers. Has a disclosed vulnerability: PT-2025-23814 / BDU:2025-06280.',
        'detail': 'SolidWall WAF is a RU WAF vendor (ООО «СолидСофт» / Solar Security). ML-based '
                  'detection with "interpreted machine learning of business level", bot protection, '
                  'business-logic attack detection. Certified for RU gov use. Available on Yandex '
                  'Cloud Marketplace. DISCLOSED VULN: PT-2025-23814 / BDU:2025-06280 (Information '
                  'Disclosure, CWE-200, CVSS v2 4.0 Medium, published 2025-02-01 by Positive '
                  'Technologies) — "Insufficient protection of service data" allows remote '
                  'unauthorised access to protected information. Also markets SolidWall AI '
                  'Security Gateway for AI-app protection and SolidPoint DAST scanner.',
        'applies_to': ['SolidWall', 'Solar Security', 'SolidSoft', 'RU WAF'],
        'indicator': "Response headers containing 'SolidWall' or 'Solid'; block pages with "
                     "Russian-language security warnings; presence of SolidWall-specific cookies.",
        'references': ['https://solidwall.io/', 'https://dbugs.ptsecurity.com/vulnerability/PT-2025-23814',
                       'https://bdu.fstec.ru/vul/2025-06280']},
    {   'id': 'fp-nemesida',
        'name': 'Fingerprint: Nemesida WAF',
        'category': 'fingerprint',
        'summary': 'Russian WAF vendor that publishes the open-source `waf-bypass` testing tool '
                   'with payloads for 12+ vuln classes.',
        'detail': 'Nemesida WAF is a RU WAF vendor. They publish an open-source WAF testing tool '
                  '(`waf-bypass`, github.com/nemesida-waf/waf-bypass) that analyses any WAF for '
                  'false positives/negatives using payloads for API, GraphQL, LDAP, LFI, NoSQLi, '
                  'RCE, RFI, SQLi, SSI, SSRF, SSTI, XSS. Available as Docker image. The payload '
                  'corpus is directly useful for offensive WAF bypass research.',
        'applies_to': ['Nemesida WAF', 'RU WAF'],
        'indicator': "Response headers containing 'Nemesida'; block pages with Russian or English "
                     "security messages; forum at forum.nemesida-waf.ru.",
        'references': ['https://github.com/nemesida-waf/waf-bypass', 'https://nemesida-waf.ru/']},
    {   'id': 'fp-pt-af',
        'name': 'Fingerprint: PT Application Firewall (Positive Technologies)',
        'category': 'fingerprint',
        'summary': 'Russian ML-based WAF by Positive Technologies — the WAF used in the PHDays '
                   'WAF Bypass contest. Already a named target in targets.yaml.',
        'detail': 'PT AF PRO is a ML-based WAF with bot protection, API security, and business-'
                  'logic attack detection. It is the WAF used in the PHDays WAF Bypass contest '
                  '(phdays.com/en/press-center/news/waf-bypass-results/) — the most concentrated '
                  'public record of PT AF bypass techniques. Each contest task = a script with a '
                  'typical vulnerability; participants compete to bypass PT AF protection.',
        'applies_to': ['PT Application Firewall', 'PT AF', 'Positive Technologies', 'RU WAF'],
        'indicator': "Response headers containing 'PT' + 'AF' or 'Positive Technologies'; block "
                     "pages with Russian security messages; presence at /ptaf/ or similar paths.",
        'references': ['https://ptsecurity.com/products/af/',
                       'https://phdays.com/en/press-center/news/waf-bypass-results/']},
    # 2026-07-24 survey: AI semantic WAF class (how-it-works)
    {   'id': 'hiw-ai-semantic-waf',
        'name': 'AI semantic-analysis WAF class',
        'category': 'how-it-works',
        'summary': 'A new WAF class that uses AI/ML to understand request semantics rather than '
                   'signature matching. Claims to capture 0-day attacks without signatures. '
                   'Changes the bypass calculus: traditional encoding/obfuscation may fail.',
        'detail': 'AI semantic-analysis WAFs (SafeLine/雷池 by Chaitin, SolidWall by Solar '
                  'Security, Cloudflare Attack Score) use machine learning to model the "intent" '
                  'of a request rather than matching it against known-bad patterns. The defensive '
                  'claim is that novel/0-day attacks are caught because their semantics are '
                  'suspicious even if their signatures are unknown. Offensive implication: '
                  'traditional payload obfuscation techniques (URL encoding, case variation, '
                  'comment insertion, double-encoding, unicode tricks) that defeat signature-based '
                  'WAFs may be LESS effective against semantic engines because the semantic model '
                  'sees through surface-level obfuscation. Bypass strategies shift toward: '
                  '(1) parser differentials — the WAF and backend parse differently (see '
                  'parser_diff_probe); (2) logic-layer attacks — business-logic flaws the WAF '
                  'does not model; (3) protocol-level attacks — HTTP smuggling, desync, chunk '
                  'extension; (4) context-based payloads — payloads that are benign in isolation '
                  'but malicious in context (second-order injection); (5) WAF burden/DoS — flood '
                  'the WAF to degrade detection (the WAF processing budget is finite).',
        'applies_to': ['SafeLine/雷池', 'SolidWall', 'Cloudflare Attack Score', 'AI WAFs'],
        'indicator': '',
        'references': ['https://safeline.pro/', 'https://solidwall.io/',
                       'https://developers.cloudflare.com/waf/detections/attack-score/']},
