...
SERVER_SIDE_VULNS: list[dict] = [   {   'id': 'broken-access-control-idor-bola',
    ...
    # 2026-07-24 survey: new vulnerability classes
    {   'id': 'postmessage-abuse',
        'name': 'postMessage abuse',
        'category': 'client-side',
        'summary': 'Exploiting window.postMessage() with insecure origin validation or '
                   'injectable message content to achieve XSS, data theft, or action '
                   'hijacking. MSRC landmark study (Aug 2025) identified 3 anti-patterns + '
                   '3 exploitation techniques. CVE-2024-49038 (Copilot Studio 0-click XSS).',
        'where_it_breaks': 'Any web app using window.postMessage() without verifying '
                           'event.origin, with wildcard (*) target origin, or where '
                           'incoming message content reaches a DOM sink (innerHTML, eval, '
                           'script injection). Also when the app sends sensitive data via '
                           'postMessage to a frame with an attacker-controllable origin.',
        'detection': 'Search JS source for postMessage(, addEventListener("message", '
                     'Check if event.origin is validated against an allowlist. Check if '
                     'message data reaches innerHTML/eval/document.write. Check if the '
                     'sender uses "*" as target origin.',
        'waf_notes': 'Client-side — WAF does not see postMessage traffic (browser-internal).',
        'notable_incidents': 'CVE-2024-49038 (Microsoft Copilot Studio 0-click XSS); '
                             '5-CVE cluster 2025-2026.',
        'root_cause': 'implicit-trust-client-metadata',
        'popularity': 'uncommon',
        'references': ['https://portswigger.net/research/postmessaged-and-compromised',
                        'https://developer.mozilla.org/en-US/docs/Web/API/Window/postMessage']},
    {   'id': 'server-side-prototype-pollution',
        'name': 'Server-Side Prototype Pollution (SSPP)',
        'category': 'injection',
        'summary': 'Polluting Object.prototype on the server (Node.js) via recursive merge '
                   'of attacker-controlled JSON into default objects. Enables property '
                   'injection, RCE via gadgets (Express options, EJS templates, data: URL '
                   'with --import flag for Node >=19).',
        'where_it_breaks': 'Node.js apps that deep-merge user input (req.body, req.query) '
                           'into config/default objects without filtering __proto__ / '
                           'constructor.prototype. Libraries: lodash.merge, '
                           'express-session config, template engines.',
        'detection': 'moonmcp_cspp_probe (client-side detection). For server-side: inject '
                     '{"__proto__":{"polluted":"yes"}} into JSON body, then check if a '
                     'subsequent response reflects the polluted property. Use --import '
                     'data: URL gadget for Node >=19 RCE.',
        'waf_notes': 'JSON body — WAF may inspect but __proto__ is valid JSON.',
        'notable_incidents': 'Node >=19 RCE via --import data: URL gadget (2026 survey).',
        'root_cause': 'code-data-confusion',
        'popularity': 'uncommon',
        'references': ['https://portswigger.net/research/server-side-prototype-pollution']},
    {   'id': 'saml-bypass',
        'name': 'SAML authentication bypass',
        'category': 'auth',
        'summary': 'Bypassing SAML signature verification via XML Signature Wrapping '
                   '(XSW), Void Canonicalization, attribute pollution, or namespace '
                   'confusion. "Golden SAML Response" always passes verification.',
        'where_it_breaks': 'SAML implementations using inconsistent XML parsers for '
                           'signature verification vs business logic (ruby-saml CVE-2025-'
                           '66567/66568, php-saml, xmlseclibs). Void Canonicalization: '
                           'relative namespace URI → empty string digest → always-valid.',
        'detection': 'XSW: inject second Signature wrapping original. Void Canonicalization: '
                     'use relative namespace URI (xmlns:ns="1") + empty-string digest. '
                     'Attribute pollution: colliding simple names. Namespace confusion: '
                     'xml:xmlns redefinition.',
        'waf_notes': 'XML body — WAF may not deeply parse SAML. Parser-differential attack.',
        'notable_incidents': 'CVE-2025-66567/66568 (ruby-saml Void Canonicalization). '
                             'CVE-2025-25291/25292 (ruby-saml XSW). GitHub Enterprise.',
        'root_cause': 'parser-differential',
        'popularity': 'uncommon',
        'references': ['https://portswigger.net/research/the-fragile-lock',
                        'https://github.com/d0ge/XSW']},
    {   'id': 'windows-ansi-bestfit',
        'name': 'WorstFit / Best-Fit Windows ANSI attacks',
        'category': 'injection',
        'summary': 'Windows "Best-Fit" char conversion transforms non-ASCII to "similar" '
                   'ASCII when using ANSI APIs. System-wide attack surface — no standard '
                   'library can prevent it (OS level). Enables PHP-CGI RCE, filename '
                   'smuggling, argument splitting, env var confusion, WAF bypass.',
        'where_it_breaks': 'Windows servers using ANSI APIs (GetCurrentDirectoryA, '
                           'FindFirstFileA, GetFullPathNameA, GetEnvironmentVariableA, '
                           'escapeshellarg in PHP). Especially CN/JP/KR code pages.',
        'detection': 'Inject non-ASCII mapping to security-relevant ASCII: soft hyphen '
                     'U+00AD→dash (CVE-2024-4577), ¥U+00A5→backslash, ＂U+FF02→quote. '
                     'Check https://worst.fit/mapping/ for full table.',
        'waf_notes': 'WAF may not perform Best-Fit → parser differential WAF vs backend.',
        'notable_incidents': 'CVE-2024-4577 (PHP-CGI RCE CN/JP). Cuckoo Sandbox RCE. '
                             'ElFinder RCE. Excel CVE-2024-49026.',
        'root_cause': 'parser-differential',
        'popularity': 'rare',
        'references': ['https://blog.orange.tw/posts/2025-01-worstfit-unveiling-hidden-transformers-in-windows-ansi/',
                        'https://worst.fit/mapping/']},
    {   'id': 'llm-prompt-injection',
        'name': 'LLM Prompt Injection',
        'category': 'injection',
        'summary': 'OWASP LLM Top 10 #1 (2025). Direct injection overrides system '
                   'instructions; indirect/XPIA injects instructions in content the LLM '
                   'ingests. Enables system prompt leakage, info disclosure, excessive agency.',
        'where_it_breaks': 'Any LLM-powered application: chatbots, copilots, AI agents, '
                           'RAG systems. Direct: user prompt overrides system instructions. '
                           'Indirect/XPIA: instructions in content the LLM reads.',
        'detection': 'Direct: "ignore previous instructions", "reveal system prompt", '
                     'DAN/STAN jailbreaks, language-switching, encoding bypass. Indirect: '
                     'inject in RAG docs, web pages, file metadata. Use '
                     'detect_prompt_injection MCP tool to test payloads.',
        'waf_notes': 'N/A — LLM layer, not HTTP layer.',
        'notable_incidents': 'CVE-2024-49038 (Copilot Studio). AI agent supply-chain '
                             'injection cases (2026). Growing HackerOne category.',
        'root_cause': 'code-data-confusion',
        'popularity': 'common',
        'references': ['https://genai.owasp.org/llm-top-10/']},
    {   'id': 'mcp-protocol-vulns',
        'name': 'MCP (Model Context Protocol) vulnerabilities',
        'category': 'api',
        'summary': 'Three protocol-level vulns: (1) absent capability attestation — '
                   'servers claim arbitrary permissions without proof; (2) bidirectional '
                   'sampling without origin authentication — servers make client LLM '
                   'generate prompts; (3) implicit trust in server-provided tool '
                   'definitions — malicious tool descriptions contain prompt injection.',
        'where_it_breaks': 'Any MCP client connecting to external MCP servers, or any '
                           'system exposing an MCP server (AI tools, agent platforms). '
                           'Tool poisoning: typosquatted or compromised MCP servers.',
        'detection': 'Enumerate MCP tools (tools/list), check auth on tool calls, check '
                     'if capabilities attested (signed) or declared. Set up rogue MCP '
                     'server, test sampling abuse. Inject prompt injection in tool '
                     'descriptions.',
        'waf_notes': 'N/A — MCP is a separate protocol layer.',
        'notable_incidents': 'arxiv 2601.17549 (2026): first MCP security analysis. '
                             'KISA H1 2025: first national CERT to analyse MCP/A2A. '
                             'JeecgBoot v3.9.1 AIRAG-MCP command-exec.',
        'root_cause': 'implicit-trust-client-metadata',
        'popularity': 'rare',
        'references': ['https://arxiv.org/html/2601.17549v1',
