...
# Foreign-key integrity check: every vuln's `root_cause` must resolve to a
# ROOT_CAUSES entry id. A typo here (e.g. 'implicit-trust-of-client-metadata'
# vs the canonical 'implicit-trust-client-metadata') silently orphans the vuln
# from the taxonomy — `by_root_cause`/`get_root_cause` miss it. This assertion
# runs at import time so the typo is caught immediately, not silently.
# (Audit 2026-07-27, KB9; the typo it catches was KB8.)
_ROOT_CAUSE_IDS = {r["id"] for r in ROOT_CAUSES}
_ORPHANED = [v["id"] for v in SERVER_SIDE_VULNS
             if v.get("root_cause") and v["root_cause"] not in _ROOT_CAUSE_IDS]
if _ORPHANED:
    raise RuntimeError(
        f"vulns_data integrity error: root_cause FK dangling for {_ORPHANED[:5]} "
        f"(len={len(_ORPHANED)}). Every SERVER_SIDE_VULNS.root_cause must match a "
        f"ROOT_CAUSES.id. Check for typos like 'implicit-trust-of-client-metadata' "
        f"vs 'implicit-trust-client-metadata'."
    )
