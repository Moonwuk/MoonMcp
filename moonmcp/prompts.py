...
OPERATING_LOOP = (
    ...
    "0. RECALL: before working a target, `memory_brief(target)` (rollup of entities/findings/\n"
    "   leads/lessons) + `memory_search(target=…)` — build on prior/other-agents' work instead\n"
    "   of re-deriving it; skip recon already done; `memory_lesson(action=recall)` to reapply\n"
    "   past tradecraft.\n"
    ...
    "   via `promote_lead`. Both mirror into shared memory (deduped, auto-linked into the\n"
    "   knowledge graph) for cross-agent reuse; use `memory_link` to connect facts\n"
    "   (host→tech, finding→cve). Once you verify or refute a lead, `label_finding` it\n"
    "   (true/false positive) so `metrics` tracks real precision; keep a short decision log.\n"
    "7. LEARN: when a technique, trap or tool quirk is worth reusing across targets, save it\n"
    "   with `memory_lesson(action=add, …)` so future sessions start ahead — don't repeat a fail.\n"
...
def bug_bounty_operator(target: str = "example.com", focus: str = "") -> str:
    ...
    return (
        ...
        "  `authz_probe` (user-A vs user-B vs anonymous diff → IDOR/BOLA),\n"
        "  `analyze_config`, `jwt_analyze`, `cors_audit`, `http_methods`.\n"
        "- Vuln surface: `cve_search`/`cve_lookup` (map software+version), `cors_audit`,\n"
        "  `graphql_check`, `takeover_check`, `open_redirect`, `desync_probe`,\n"
        "  `cspp_probe` (client-side prototype pollution, headless-browser, safe). When a WAF\n"
        ...
        "  `technique_info` (with `query=` to search) for exploitation techniques & landmark PoCs.\n"
...
def injection_hunt(target: str = "example.com", injection_class: str = "") -> str:
    ...
    return (
        ...
        f"\n{_FALSE_POSITIVE_RULE}"
        "\n6. On a confirmed class, consult `technique_info` (with `query=` to search) for the\n"
...
def technique_advisor(technology: str = "", cve: str = "") -> str:
    ...
    return (
        ...
        f"\n{RULES_OF_ENGAGEMENT}"
        "\nHOW TO ADVISE:\n"
        f"- Query the catalog: `technique_info(query=…)` for `{subj}` and `cve_search` for its\n"
...
def business_logic_hunt(target: str = "example.com", flow: str = "") -> str:
    ...
    return (
        ...
        "  • IDOR / authorization — swap an object id / user id (also `authz_probe`); "
