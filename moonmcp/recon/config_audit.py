...
# Cap on XML content size and entity-expansion budget — guards against the
# billion-laughs / XML-bomb DoS in ElementTree.fromstring (audit 2026-07-18,
# bug #5). A target serving a large or deeply-nested XML config can't blow up
# the parser: oversized inputs fall back to the generic KV scraper.
_XML_MAX_BYTES = 1_000_000  # 1 MB ceiling — configs are small
_XML_MAX_DEPTH = 50         # nesting ceiling
...
def _parse_xml(content: str) -> list[tuple[str, str]]:
    out: list[tuple[str, str]] = []
    # Size guard: a >1 MB XML blob is not a config file — refuse to parse it
    # rather than risk a billion-laughs expansion or a deeply-nested tree.
    if len(content) > _XML_MAX_BYTES:
        return _parse_generic(content)
    # Reject internal DTDs / ENTITY declarations outright — ElementTree does
    # not expose expat's entity-handler hooks, so the only safe stance is to
    # not parse documents that define entities at all.
    if re.search(r"<!ENTITY\b", content, re.IGNORECASE):
        return _parse_generic(content)
    try:
