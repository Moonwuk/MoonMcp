...
from .reporting import TOOL_NAME

_SLUG = re.compile(r"[^a-z0-9]+")
_SEV_COLOR = {"critical": "1", "high": "2", "medium": "3", "low": "5", "info": "6"}

# Tag slug for frontmatter — lowercase, suitable as an Obsidian tag.
_TAG = re.sub(r"[^a-z0-9]+", "-", TOOL_NAME.lower()).strip("-") or "tool"
...
def _graph_report(nodes: list[dict], links: list[dict]) -> str:
    ...
    lines = [f"# {TOOL_NAME} graph report", "",
    ...
    *,
    ...
    for v in (vulns or []):
        ...
        fm = frontmatter({"type": "vuln", "id": v["id"], "category": v.get("category"),
                          ...
                          "root_cause": rc_id,
                          "tags": [_TAG, "kb", "vuln", f"cat/{v.get('category', 'other')}"]})
    ...
    vault.write(f"{TOOL_NAME} Home.md", "\n".join(home))
    ...
    if want_canvas:
        ...
        vault.write(f"{TOOL_NAME}.canvas", _canvas(nodes, edges))
    ...
    if want_graph_json:
        ...
        vault.write("graph.json", json.dumps(
            {"directed": True, "multigraph": False, "graph": {"generator": _TAG},
