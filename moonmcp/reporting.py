...
# The tool name published in SARIF `driver.name` and the report footer. Default
# is "MoonMCP" but is overridable via MOONMCP_TOOL_NAME so an operator who does
# not want to advertise the tool in reports submitted to a program can rename
# it (or blank it) without code changes (audit 2026-07-27, debt #6).
TOOL_NAME = os.environ.get("MOONMCP_TOOL_NAME", "MoonMCP") or "MoonMCP"
...
def format_markdown(report: dict, *, generated_at: str | None = None) -> str:
    ...
    lines: list[str] = [f"# {TOOL_NAME} recon report — `{target}`", ""]
    if generated_at:
    ...
    lines += ["---", "", f"_{TOOL_NAME} — authorised testing only. Verify findings before reporting._"]
    return "\n".join(lines)
