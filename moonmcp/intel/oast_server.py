...
class CallbackServer:
    ...
    Bind default is ``127.0.0.1`` (loopback) so the listener is never exposed
    to the LAN/internet by accident — a cloud host with ``0.0.0.0`` would open
    the catcher to the world with no auth. To accept callbacks from a remote
    target, pass ``host="0.0.0.0"`` **and** an explicit ``advertise_host``
    (the public/reachable IP the canary URLs should point at); binding
    ``0.0.0.0`` without an advertise host is a silent misconfiguration
    (listener open but canaries point at loopback → unreachable), so it is
    refused here.
    """

    def __init__(self, host: str = "127.0.0.1", port: int = 0,
                 advertise_host: str | None = None) -> None:
        # Refuse the silent-misconfiguration combo: binding all interfaces
        # without an explicit advertise_host would open the listener AND make
        # canaries point at 127.0.0.1 (unreachable by the target). Require the
        # operator to state the reachable address explicitly.
        if host in ("0.0.0.0", "") and not advertise_host:
            raise ValueError(
                "binding 0.0.0.0 requires an explicit advertise_host (the "
                "reachable IP for canary URLs); otherwise the listener is open "
                "but canaries point at loopback and never receive callbacks. "
                "Pass advertise_host=<public-ip>, or use the default 127.0.0.1."
            )
        self._host = host
