...
class ScopeManager:
    ...
    def blocked_connect_reason(self, target: str) -> str | None:
        ...
        Thin wrapper over :meth:`resolve_pin`, which does the single resolve-and-vet;
        callers that also want the pinned address to dial should use ``resolve_pin``
        directly rather than re-resolving.
        """

        return self.resolve_pin(target)[0]
