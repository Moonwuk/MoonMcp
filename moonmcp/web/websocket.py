...
_FOREIGN_ORIGIN = "https://x-cswsh-probe.example"

# C0 control characters (incl. CR/LF) that must never appear in a raw HTTP
# header line built by build_handshake — a target-controlled subprotocol (or
# path, host, origin, extra-header value) with embedded CR/LF is a header
# injection vector over the raw socket (audit 2026-07-18, bug #3).
_CTRL = "".join(chr(i) for i in range(32)) + "\x7f"


def _strip_ctrl(value: str) -> str:
    """Remove C0 control chars (incl. CR/LF) — keep header values on one line."""
    return value.translate(str.maketrans("", "", _CTRL))
...
def build_handshake(host_header: str, path: str, key: str, *, origin: str | None = None,
    ...
    Every field that flows into a header line is stripped of C0 control chars
    (incl. CR/LF) so a target-controlled ``subprotocol`` / ``path`` / ``origin``
    / extra-header value cannot inject a new line over the raw socket.
    """

    lines = [
        f"GET {_strip_ctrl(path or '/')} HTTP/1.1",
        f"Host: {_strip_ctrl(host_header)}",
        "Upgrade: websocket",
        "Connection: Upgrade",
        f"Sec-WebSocket-Key: {_strip_ctrl(key)}",
        "Sec-WebSocket-Version: 13",
    ]
    if origin:
        lines.append(f"Origin: {_strip_ctrl(origin)}")
    if subprotocols:
        lines.append(f"Sec-WebSocket-Protocol: {_strip_ctrl(subprotocols)}")
    for k, v in (extra_headers or {}).items():
        lines.append(f"{_strip_ctrl(str(k))}: {_strip_ctrl(str(v))}")
    return ("\r\n".join(lines) + "\r\n\r\n").encode("latin-1")
