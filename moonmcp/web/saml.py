...
# input from the target) with a billion-laughs / quadratic-entity payload can
# exhaust CPU/memory in this process. defusedxml blocks entity expansion by
...
# independent of entity expansion) or a misuse. Cap the decoded payload and
# the parse input so a malicious response can't blow up the process.
# (Audit 2026-07-27, WA5 + WA6 — the deepcopy in build_variant is bounded
# by this cap on the parsed tree size.)
_MAX_DECODED_BYTES = 256 * 1024   # 256 KiB — generous for any real SAML doc
_MAX_PARSE_BYTES = 256 * 1024     # 256 KiB — cap before fromstring
...
DEFAULT_FORGED_NAMEID = "x-xsw-forged@internal"
...
def decode_response(raw: str) -> str:
    ...
    The decoded payload is capped at ``_MAX_DECODED_BYTES`` so a multi-MB
    base64 blob can't amplify a downstream entity-expansion DoS or exhaust
    memory. An oversized payload returns a short error string instead.
    """

    text = raw.strip()
    if text.startswith("<"):
        if len(text) > _MAX_DECODED_BYTES:
            return "<error>oversized</error>"
        return text
    try:
...
def parse_structure(xml_text: str) -> dict:
    ...
    The input is capped at ``_MAX_PARSE_BYTES`` and parsed with defusedxml
    (no entity expansion) so a malicious SAMLResponse can't exhaust the
    process via billion-laughs or a huge document tree.
    """

    if len(xml_text) > _MAX_PARSE_BYTES:
        return {"error": "oversized", "detail": f"input > {_MAX_PARSE_BYTES} bytes"}
    try:
