...
_CANARY_HOST = "x-behaviour-canary.example"
...
async def profile_behavior(client: HttpClient, url: str, *, scope_check=None) -> BehaviorProfile:
    ...
    # 404 handling
    nf = await client.fetch(url.rstrip("/") + "/x-does-not-exist-9f2c",
    ...
    for path in ("/%c0%ae%c0%ae/", "/?moonmcp[]=1", "/'\"><"):
        ...
        for sig in _ERROR_SIGNATURES:
            # Only count a signature the fuzz input INTRODUCED. Compare OCCURRENCE COUNTS, not
            # mere presence: a token already on the normal page (a docs / tutorial / blog site —
            # or MoonMCP's own report — mentioning "stack trace" / "Warning:" / "django.") is
            # ambient content, but if the fuzz response carries MORE of it than the baseline, the
            # extra occurrence is a real leaked error (which a plain `sig not in base_body` would
            # have wrongly suppressed whenever the same token also appeared as ambient text).
            if body.count(sig) > base_body.count(sig) and sig not in result.error_disclosure:
