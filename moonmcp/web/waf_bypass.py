...
Sends **benign canary** payloads (each contains a neutral marker and does
...
# category -> benign canary payload. The marker is a neutral random-looking
# token (no tool name) so the probe doesn't fingerprint MoonMCP in target/WAF
# logs. (Audit 2026-07-27, fingerprint sweep.)
_MARKER = "x9k2q7"
_PAYLOADS = {
    "xss": f"<script>{_MARKER}(1)</script>",
    "sqli": f"1' OR {_MARKER}='{_MARKER}",
    "lfi": f"../../../../{_MARKER}/etc/passwd",
    "rce": f";{_MARKER};whoami",
    "ssti": f"{{{{7*{_MARKER}}}}}",
    "traversal": f"....//....//{_MARKER}",
    "xxe": f"<!DOCTYPE {_MARKER} [<!ENTITY x SYSTEM 'file:///{_MARKER}'>]>",
}
...
async def _send(client: HttpClient, url: str, value: str, scope_check) -> tuple[int | None, bool]:
    sep = "&" if "?" in url else "?"
    probe = f"{url}{sep}q={quote(value, safe='%')}"
