...
_INVALID_CONTROL = "x_zzz_invalid"
...
async def probe_mass_assignment(client: HttpClient, url: str, *, method: str = "POST",
    ...
    **State-changing:** this probe POSTs ``{"role":"admin","is_admin":"true",
    "balance":"999999",...}`` — if the endpoint is a real user-update and is
    vulnerable, the privileged fields are already persisted by the time the
    finding says "verify it persisted". ``dry_run=True`` (default) refuses to
    send the POST and returns a ``dry_run`` verdict describing what would
    happen; set ``dry_run=False`` to run the actual probe.
    (Audit 2026-07-27, WL2.)
    """

    if dry_run:
        return [{
            "kind": "mass_assignment", "field": "(privileged set)",
            "value": "", "severity": "info", "verdict": "dry_run",
            "detail": "would POST privileged fields (role/is_admin/balance/...) to "
                      f"{url}; re-run with dry_run=False to actually test (WARNING: on "
                      "a vulnerable user-update the privileged fields are persisted "
                      "before you can verify — no rollback).",
        }]
...
async def probe_race(client: HttpClient, url: str, *, method: str = "POST",
    ...
    **State-changing:** this probe fires *n* real requests at a state-changing
    endpoint — on a vulnerable should-be-once action (coupon, vote, withdrawal,
    invite, signup) it actually performs the action up to *n* times with no
    rollback. ``dry_run=True`` (default) refuses to fire and returns a
    ``dry_run`` verdict describing what would happen; set ``dry_run=False`` to
    run the actual race. (Audit 2026-07-27, WL1.)
    """
    ...
    if dry_run:
        return {
            ...
            "verdict": "dry_run",
            "detail": f"would fire {n} parallel {method} requests at {url}; re-run "
                      "with dry_run=False to actually test (WARNING: on a vulnerable "
                      "should-be-once action this performs it up to N times — real "
                      "side effect with no rollback).",
        }
    m = method.upper()
