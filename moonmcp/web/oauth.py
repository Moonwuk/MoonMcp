...
_OAUTH_CANARY = "x-oauth-canary.example"
...
async def probe_redirect_uri_bypass(client: HttpClient, authorization_endpoint: str, *,
    ...
    The ``authorization_endpoint`` is scope-checked before any request: a
    caller-supplied endpoint is an SSRF vector otherwise (audit 2026-07-18,
    bug #2) — ``@``-userinfo or a foreign host would route the probe to an
    attacker-controlled server.
    """

    ep = authorization_endpoint
    # Hard gate: the endpoint itself must be in scope. The `@`-userinfo trick
    # (https://legit.com@evil.com) and foreign-host endpoints are refused here,
    # before the fetch. Let the caller discover the endpoint from `target`
    # (the safe path in oauth_redirect_probe) or add it to scope first.
