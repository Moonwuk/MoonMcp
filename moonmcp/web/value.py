...
async def probe_value_tampering(client, url: str, field: str, *, method: str = "GET",
    ...
    ctrl = await _baseline(client, url, field, "x_zzz_invalid", m, scope_check)
...
async def probe_coupon_reuse(client, url: str, field: str, code: str, *, times: int = 3,
    ...
    **State-changing:** this probe actually applies the coupon/code up to `times`
    times — on a vulnerable single-use coupon that is real financial loss with
    no rollback. `dry_run=True` (default) refuses to send the repeated applies
    and returns a `dry_run` verdict describing what would happen; set
    `dry_run=False` to run the actual probe. (Audit 2026-07-27, WL7.)
    """

    if dry_run:
        return {
            "kind": "coupon_reuse", "field": field, "code": code,
            "verdict": "dry_run",
            "detail": f"would apply '{code}' to {field} {times}× sequentially; "
                      "re-run with dry_run=False to actually test (WARNING: on a "
                      "vulnerable single-use coupon this redeems it N times — real "
                      "financial loss with no rollback).",
        }
    m = method.upper()
    n = max(2, min(times, 5))
